...
#欄位8:詞頻(郵件關鍵字) Term Frequency
	jieba_zh='C:\\Users\\user\\Anaconda3\\Lib\\site-packages\\jieba\\dict.txt.big.txt'
...
"""
	SQLFunc="select dbo.fn_GetResearchFilename(?,?,?,?,?)"
	cursor.execute(SQLFunc,Broker_id,Stock_code,Report_type,Industry,Eml_datetime)
	rows=cursor.fetchall()
	cnxn.commit()
